...
### Provider Expansion & Refinement

| Feature                               | Description                                                                                                                               | Priority |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| **Google Veo refinements**            | Expand the shipped Google Veo provider with richer frame/cost reporting and tighter dashboard presentation for video-generation workloads | High     |
| **AWS Bedrock**                       | Monitor cross-model usage via Bedrock billing API (Claude, Llama, Mistral, Titan on AWS)                                                  | High     |
| **Azure OpenAI metering refinements** | Build on the shipped dedicated Azure provider with Azure metering/billing endpoint refinements and richer deployment-level reporting      | Medium   |
...
Review these risks whenever a new release scope is activated.
Treat them as release gates when planning new provider, browser-sync, or distribution work.
